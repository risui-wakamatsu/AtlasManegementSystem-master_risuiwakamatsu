...
class CalendarWeekDay
{
  ...
  function selectPart($ymd)
  {
    ...
    //ここから予約画面の予約プルダウン 枠がなかったらの場合のif文
    $html = [];
  ...
  function everyDay() //日付フォーマット
  {
    return $this->carbon->format('Y-m-d');
  }

  function authReserveDay() //setting_reserveカラム(開講日)を配列で取得
  {
    return Auth::user()->reserveSettings->pluck('setting_reserve')->toArray(); //ログインユーザー→中間テーブル→中間テーブルからsetting_reserveカラムだけ取り出す→配列に変換
  }
